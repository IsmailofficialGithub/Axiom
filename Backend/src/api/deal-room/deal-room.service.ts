...
const verifyOpportunityOwnership = async (userId: string, userRole: string, opportunityId: string) => {
  ...
  if (error || !data) throw new ApiError(404, 'Opportunity not found');
  if (data.created_by !== userId) throw new ApiError(403, 'You do not have permission to manage this deal room');
  
  return true;
};

export const addDocument = async (userId: string, userRole: string, opportunityId: string, data: any) => {
...
export const listDocuments = async (opportunityId: string, userRole: string, userId: string) => {
  ...
  if (userRole === 'startup' || userRole === 'admin') {
    ...
    if (userRole === 'startup') await verifyOpportunityOwnership(userId, userRole, opportunityId);
...
export const grantAccess = async (userId: string, userRole: string, opportunityId: string, investorId: string) => {
  await verifyOpportunityOwnership(userId, userRole, opportunityId);
  ...
  if (error) {
    if (error.code === '23505') throw new ApiError(409, 'Investor already has access');
