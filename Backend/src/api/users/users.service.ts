...
export const updateUserProfileDetails = async (userId: string, role: string, data: any) => {
  if (role === 'investor') {
    const { investment_min, investment_max, preferred_industries } = data;
    ...
    if (!existing) {
      const { data: newInvestor, error: insertError } = await supabaseAdmin
        ...
        .select()
        .single();
      if (insertError) throw new ApiError(500, `Failed to initialize investor preferences: ${insertError.message}`);
      return newInvestor;
    }
    ...
    if (updateError) throw new ApiError(500, `Failed to update investor preferences: ${updateError.message}`);
    return updatedInvestor;
  }

  if (role === 'startup') {
    const { 
      industry, stage, current_arr, last_year_revenue, revenue_model,
      funding_sought, primary_use_of_funds, previous_funding, custom_qa, description
    } = data;
    ...
    if (!existing) {
      const { data: newStartup, error: insertError } = await supabaseAdmin
        ...
        .select()
        .single();
      if (insertError) throw new ApiError(500, `Failed to initialize startup profile: ${insertError.message}`);
      return newStartup;
    }
    ...
    if (updateError) throw new ApiError(500, `Failed to update startup profile: ${updateError.message}`);
